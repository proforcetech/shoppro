...
var RolesGuard = function () {
    ...
    var RolesGuard = _classThis = /** @class */ (function () {
        ...
        RolesGuard_1.prototype.canActivate = function (context) {
            var requiredRole = this.reflector.get('role', context.getHandler());
            if (!requiredRole)
                return true;
            var request = context.switchToHttp().getRequest();
            var user = request.user;
            if (!user || user.role !== requiredRole) {
                throw new common_1.ForbiddenException("Access denied for role: ".concat(user === null || user === void 0 ? void 0 : user.role));
            }
            return true;
        };
        return RolesGuard_1;
